            ...
            <div className="grid lg:grid-cols-2 gap-8 items-center">
              ...
              <div>
                ...
                <p className="text-text-muted text-lg mb-6 leading-relaxed">
                  {featuredGame.description} Try it now — takes 2 seconds to load.
                </p>
                <div className="flex flex-wrap gap-2 mb-6">
                  <span className="px-3 py-1 bg-accent-violet/20 text-accent-violet rounded-full text-sm font-medium">
                    {featuredGame.category}
                  </span>
                  {featuredGame.tags.slice(0, 3).map((tag) => (
                    <span key={tag} className="px-3 py-1 bg-surface border border-border text-text-muted rounded-full text-sm">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="relative aspect-video rounded-xl overflow-hidden border border-border shadow-2xl shadow-accent-violet/20">
                ...
                <div className="absolute bottom-4 left-4 right-4">
                  <div className="flex items-center gap-2 text-sm text-text-muted">
                    ...
                    {featuredGame.controls || 'Click to play'}
                  </div>
                </div>
              </div>
            </div>
      ...
      <section className="bg-surface border-y border-border">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-wrap items-center justify-center gap-8 text-center">
            <div>
              <div className="font-display text-2xl font-bold text-accent-violet">{html5Games.length}+</div>
              <div className="text-xs font-mono text-text-muted">Free Games</div>
            </div>
            <div className="w-px h-8 bg-border hidden sm:block" />
            <div>
              <div className="font-display text-2xl font-bold text-accent-cyan">{categories.length - 1}</div>
              <div className="text-xs font-mono text-text-muted">Genres</div>
            </div>
            <div className="w-px h-8 bg-border hidden sm:block" />
            <div>
              <div className="font-display text-2xl font-bold text-accent-amber">Zero</div>
              <div className="text-xs font-mono text-text-muted">Cost</div>
            </div>
            <div className="w-px h-8 bg-border hidden sm:block" />
            <div>
              <div className="font-display text-2xl font-bold text-emerald-500">Instant</div>
              <div className="text-xs font-mono text-text-muted">Play</div>
            </div>
          </div>
        </div>
      </section>
